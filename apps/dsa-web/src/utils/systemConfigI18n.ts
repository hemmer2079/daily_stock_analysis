...
const fieldDescriptionMap: Record<string, string> = {
  ...
  AGENT_SKILLS: '逗号分隔的交易技能列表，例如：bull_trend,ma_golden_cross,shrink_pullback。',
  AGENT_SKILL_DIR: '存放 Agent 技能 YAML 文件的目录路径。',
  AGENT_ARCH: "选择 Agent 执行架构。single 为经典单 Agent；multi 为多 Agent 编排（实验性）。",
  AGENT_ORCHESTRATOR_MODE: "Multi-Agent 编排深度。quick（技术→决策）、standard（技术→情报→决策）、full（含风控）、specialist（含技能专家评估）。",
  ...
  AGENT_SKILL_AUTOWEIGHT: "根据回测表现自动调整技能权重。",
  AGENT_SKILL_ROUTING: "技能选择方式。auto 按市场状态自动选择，manual 使用 AGENT_SKILLS 列表。",
